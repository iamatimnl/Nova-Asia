...
db_url = "postgresql://novaasia_db_user:8PpdenNvRzaMdrpbW3WyKsWFyKxTvL9I@dpg-d13t4rumcj7s738e0qtg-a.oregon-postgres.render.com/novaasia_db"
engine = create_engine(db_url)
app.config['SQLALCHEMY_DATABASE_URI'] = "postgresql://novaasia_db_user:8PpdenNvRzaMdrpbW3WyKsWFyKxTvL9I@dpg-d13t4rumcj7s738e0qtg-a.oregon-postgres.render.com/novaasia_db"
app.config['SQLALCHEMY_TRACK_MODIFICATIONS'] = False  # 可选：关闭警告
...
@app.route('/')
def serve_index():
    return send_from_directory('electron-app', 'index.html')

@app.route('/map')
def show_map():
    return render_template('map.html')  # 从 templates 加载

# 可选：只为特定静态资源开启路由
@app.route('/assets/<path:filename>')
def serve_assets(filename):
    return send_from_directory('electron-app', filename)
...
class Order(db.Model):
    ...
    discountAmount = db.Column('discountAmount', db.Float)
...
def api_orders():
    try:
        ...
        if order.discountCode and str(order.discountCode).upper() == "KASSA":
            order.discountCode = "kassa korting"

        # 4. 保存订单到数据库
        db.session.add(order)
        db.session.commit()

        # 5. 如有新折扣码，记录到 discount_codes 表
        customer_email = (
            data.get("customer_email")
            or data.get("customerEmail")
            or order.email
        )
        new_code = order.discount_code
        new_amount = order.discount_amount or 0

        if new_code and customer_email:
            disc = DiscountCode(
                code=new_code,
                customer_email=customer_email,
                discount_percentage=3.0,
                discount_amount=new_amount,
                is_used=False,
            )
            db.session.add(disc)
            db.session.commit()
            print(
                f"✅ 折扣码保存成功: {new_code} for {customer_email} met korting {new_amount}"
            )
...
def pos_orders_today():
    ...
    for o in orders:
        ...
        # --- 金额推导（保留你的思路，但别回写 o 的字段，避免副作用）---
        totaal = o.totaal or 0.0
        subtotal = sum(float(i.get("price", 0)) * int(i.get("qty", 0))
                       for i in o.items_dict.values())

        # 严格分流：本次 vs 下次
        this_code = getattr(o, "discountCode", None)
        this_amt  = getattr(o, "discountAmount", None)

        next_code = getattr(o, "discount_code", None)
        next_amt  = getattr(o, "discount_amount", None)
        ...
        btw_21 = o.btw_21
        btw_9 = o.btw_9
        if btw_total is None:
            heineken_total = sum(
                float(it.get("price", 0)) * int(it.get("qty", 0))
                for name, it in o.items_dict.items()
                if "heineken" in str(name).lower()
            )
            if btw_21 is None:
                btw_21 = round(heineken_total * 0.21, 2)
            if btw_9 is None:
                base_total = subtotal - heineken_total + verp + delivery
                btw_9 = round(base_total * 0.09, 2)
            btw_total = (btw_9 or 0.0) + (btw_21 or 0.0)
        ...
        order_dicts.append({
            ...
            "totaal": totaal,
            "total": totaal,
            "verpakkingskosten": verp,
            "bezorgkosten": delivery,
            "packaging_fee": verp,
            "delivery_fee": delivery,
            "btw_9": btw_9 or 0.0,
            "btw_21": btw_21 or 0.0,
            ...
            "discountAmount": float(this_amt) if this_amt is not None else 0.0,

            # ★ 下次奖励（snake_case）——只读 discount_code/discount_amount
            "discount_code": next_code,
